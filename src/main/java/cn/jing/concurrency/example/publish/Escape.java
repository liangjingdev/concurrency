...
/**
 * function:对象溢出 (在对象构造完成之前，不可以将其发布)
 * 
 * @author liangjing
 *
 */
...
public class Escape {
	...
	/**
	 * function:包含了对封装实例的隐藏和引用，这样在对象没有被正确构造完成之前就会被发布，由此导致不安全的因素在里面
	 * 1.导致this引用在构造期间溢出的错误，他是在构造函数构造过程中启动了一个线程，造成this引用的溢出
	 * 新线程只是在对象构造完毕之前就已经看到他了，所以如果要在构造函数中创建线程，那么不要启动它， 而是应该使用一个专有的start，或是其他的方式统一启动线程
